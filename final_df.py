...
df = df[["fashion_house", "show", "URL", "cover_image_url", "year", "category","season","location","description","editor","publish_date","designer_name","image_urls"]]

df = df.explode("designer_name")
# Remove None or empty strings if any
df = df[df["designer_name"].notna() & (df["designer_name"] != "")]

bio_brands = pd.read_json("data/brand_data_fmd.json", lines=True)
